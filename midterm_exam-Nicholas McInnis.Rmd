...
The confidence interval involves the creation of an interval, using the sample mean, z-score and standard error, that is likely to contain the population mean (mu) at a certain level of confidence (e.g. 95%). For example, for a given confidence interval (confidence level=95%), "we are 95% confident that this confidence interval is likely to contain the true population mean".
...
In statictics, hypothesis testing is used to determine if a null hypothesis (Ho), representing the status quo, should be rejected or not in favor of an alternative hypothesis (H1), representing the relationship researchers are trying to prove. Sample data is used to develop a model and this model produces a test-static (usually F-stat or t-stat). Should the absolute value of this test-stat cross a certain significance level threshold in the normal distribution (based on confidence level), the model is statistically significant and the null hypothesis is rejected in favour of the alternative hypothesis.
...
The objective of this article was to determine the effect of firm age (x1), knowledge intensity (x2), and the imitability of the firm's technology (x3) on the growth in international sales of born globals (y). This can be reformulated in the following research question: "What is the effect of firm age, knowledge intensity, and technology immitability on the growth of international sales of born-globals?" Based on the hypotheses, this RQ can be formulated into the following equation:

y-hat = B0 - B1*x1 + B2*x2 - B3*x3

In this model, the researchers controlled for variables of Legal Protection, Growth orientation, Firm age, international sales before 1992, DOI in 1992, and total sales in 1992, relevant control variables because they are all before the 5-year window of data collection.
To investigate this RQ, the researchers collected panel data from 59 high-tech born-global firms in Finland over 5 years. In terms of results, the researchers were able to confirm their hypotheses for firm age and knowledge intensity. However, they found a positive relationship between tech immitability (x3) and growth in international sales (y) instead of the hypothsized negative relationship. The main contribution of this article is "the learning advantage of knewness", which explains how born-global firms that are knowledge intensive can better understand international expasion issues than non-knowledge intensive born-global firms, and can thus generate higher international sales growth. This also supports the hypothesis for knowledge intensity (x2)

In terms of accuracy of the models used, Models 1 and 3 demonstrate the highest adjusted R-squared, though Model 3 is more significant because its F-test result statisitc is higher than that of Model 1. 

As regards model validity, we would need to see the underlying data and residuals to assess linearity, normal distribution of residuals, and constant variance. However, from the tables we have available, we can suspect multicollinearity in the results. More specifically, in Table 1, we notice a correlation between knowledge intensity (x2) and immitability (x3) of -0.38 with a p-value of less than 0.01. This p-value is less than 0.05, which indicates that this correlation is staistically significant and suggest that there is multicollinearity among variables x2 and x3 of the model, which impacts the validity of the model. Qualitatively, this makes sense because the more knowledge-intensive a firm is, the less imitable are the technology it produces, because it is hard to imitate the knowledge behind the technology.

Hence, I would recommend revising and resubmitting with minor suggestions to correct the issue of multicollinearity. I would recommend verifying th findings with three linear models, one for each independent variable, because the combination of the three variables in the same model induces collinearity.

Lastly, I would also suspect a survivorship bias among the firms, in that it is very difficult to collect data about the independent vairables in old firms to determing their growth in international sales if they have declared bankruptcy. Therefore, the data collected may not accurately reflect the population of high-tech-firms in Finland.
...
```{r, eval = FALSE}
# Here, I am using bonus as my dependent variable (y) and HDI as my independent variable (x1)
m4 <- lm(mydata1$bonus ~ mydata1$HDI)

# I am using the summary function to see the Adjusted R-squared
summary(m4)
```
...
Using the summary, we can see how the Adjusted R-square is relatively low, meaning that the model doesn't explain the bonus well. However, the HDI variable alone is very accurate, in that it generates a very low p-value, though more variables are needed to construct a more accurate model.
...
```{r, eval = FALSE}
# Concept 1: Linearity, I want to see if the model follows a linear relationship. This can be done by creating a scatterplot of the data or a residuals plot.

plot(m4$residuals)
  
# Although the relationship between y and x is weak, we can indeed observe a linear relationship in the dataset
```
...
# Concept 2: Nearly normal residuals. Here, I want to see if the model's residuals follows a normal distribution. This can be observed by creating a histogram of the residuals.
  
hist(m4$residuals)

# By observing the histogram, the residuals appear to be normally distributed around the mean. Thus, the model is valid under this concept.
...
```{r, eval = FALSE}
# Concept 3: Constant variability/homoscedasticity. Here, we want to see if the data/residuals are homoscedastic, meaning that they vary constantly. We can observe this by making a scatterplot of the data or a residuals plot

plot(m4$residuals)

# Using the residuals plot, we can observe that residuals do vary constantly, and are therefore homoscedastic. Hence, the model is valid under this concept.
...
# In this new equation(m5), y= bonus (independent variable), x1= hdi and x2= graduate2 (dependent variables. x1 is a numerical variable while x2 is a dummy variable.

m5 <- lm(mydata1$bonus ~ mydata1$HDI + mydata1$graduate2)
  
# Model accuracy by refering to the summary, R-squared etc. 
summary(m5)   

# Explanation: Given the low adjusted R-squared (0.143), the models isnt very effective at explaining y. However, the coefficients are quite significant given their low p-values, notably HDI.
...
# In comparison, m4 and m5 both have very low adjusted R-squared values (0.1374 and 0.143 respectively), though m5 is slightly more accurate with the adde independent variable. However, since these are nested models, the best comparison is the F-test, which accounts for degrees of freedom. M4 has an F-stat of 76.83 whereas m5 has an F-stat of 40.72, and thus comaratively, m4 is the more accurate model.
...
```{r, eval = FALSE}
# here, I am making a model for the new independent variables mentioned about, and their ability to explain bonus

m6 <- lm(mydata1$bonus ~ mydata1$sexofamb + mydata1$time)
  
summary(m6)  

# Explanation, this model is very accurate at describing the bonuses (Adjusted r-quare of 0.7834). In addition, the independent variables are very accurate givent their very low p-values.
...
(women) y-hat  = 860 + 4039(1) + 450(time)
              = 4899 + 450(time)

(men) y-hat = 860 + 4039(0) + 450(time)
              = 860 + 450(time)


**What can you conclude?**

Here, we see that men and women are not paid equally, and that women are paid more.
...
# here, I multiply the independent var. to create the interaction variable
m7 <- lm(mydata1$bonus ~ mydata1$sexofamb*mydata1$sexofsecr )
  

summary(m7)  

# based on the summary, the interaction variable is highly significant, which suggests that there may be collinearity.
...
Next, I would test for multicolinnearity between the independent vairables, as this would describe the validity of the model even further. I would also include a quartatice variable and use the summary function to see if this quadratic variable is more significant than the linear variable, which could suggest that there is a quadratic effect in the equation for buns.


I woula also try more models with different combinations o independent variables to find the model with the highest adjusted are squared.
